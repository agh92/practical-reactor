...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void mail_box_switcher() {
        Flux<Message> myMail = mailBoxPrimary().switchOnFirst((signal, flux) -> signal.get().metaData.equals("spam") ? mailBoxSecondary() : flux);
    ...
    public void prettify() {
        Mono<Void> fileWritten = openFile()
                .then(writeToFile("0x3522285912341"))
                .and(closeFile());

        Mono<Boolean> successful = Mono.when(fileWritten).thenReturn(true);
    ...
    public void car_factory() {
        Flux<Car> producedCars = carChassisProducer().zipWith(carEngineProducer()).map((tuple) -> new Car(tuple.getT1(), tuple.getT2()));
    ...
    public Mono<String> chooseSource() {
        // use defer to make de decision at subscription time and not call time
        return Mono.defer(() -> {
            String source = sourceRef.getAcquire();
            if (source.equals("A")) {
                return sourceA();
            }
            if (source.equals("B")) {
